...
CSV_PATH = OUT_DIR / "external_validation_ni_fg_d5_calibration_v1.csv"
JSON_PATH = OUT_DIR / "external_validation_ni_fg_d5_calibration_v1.json"
...
def run():
    ...
    for scenario in SCENARIOS:
        ...
        seed = stable_seed(
            BASE_SEED,
            "fg_d5_boundary_calibration",
            scenario["id"],
        )

        result = simulate_fg_d5_boundary_calibration(
        ...
        rows.append(
            {
                ...
                "reference_min_cluster_size": ref["min"],
                "reference_max_cluster_size": ref["max"],
                "comparison_min_cluster_size": cmp_["min"],
                "comparison_max_cluster_size": cmp_["max"],
                ...
                "df_min": result.min_df,
                "df_median": result.median_df,
                "df_max": result.max_df,
            }
        )
    ...
    payload = {
        "screen_id": "external_validation_ni_fg_d5_calibration_v1",
        "status": "precollection_candidate_method_calibration_not_final",
        "method": (
            "Fay-Graubard d5: FG bias-corrected sandwich plus "
            "Fay-Graubard data-dependent dtildeH degrees of freedom"
        ),
        "implementation_provenance": (
            "Python algebraic port of the public saws 0.9-7.0 "
            "sawsCalc/PsiTildeCalc/dfCalc formulas, specialized to the "
            "two-cell clustered identity-link marginal risk-difference model"
        ),
        "primary_margin": 0.03,
        "one_sided_alpha": 0.025,
        "repetitions_per_scenario": REPETITIONS,
        "important_boundary": (
            "This screen does not freeze FG-d5, sample sizes, cluster "
            "minima/caps, absolute FNR ceiling, or W0."
        ),
        "csv": str(CSV_PATH),
    }
    ...
    print()
    print("=== FAY-GRAUBARD d5 20,000-REPETITION TYPE-I CALIBRATION ===")
    ...
    for row in rows:
        print(
            ...
            f"[{row['df_min']:.2f}, {row['df_max']:.2f}]"
        )

    worst = max(rows, key=lambda row: row["type_I_pass_rate"])
    print()
    print(
        f"FG-d5 MAX TYPE-I: {worst['type_I_pass_rate']:.5f} "
    ...
    print()
    print(
        "Do not freeze W0 from this screen. If FG-d5 controls the asymmetric "
        "stress cases substantially better than KC/MD, we can promote it to "
        "the final-method candidate and then rerun power/resource design."
    )
